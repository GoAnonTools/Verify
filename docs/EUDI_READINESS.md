...
2. GoAnon asks for an age-threshold proof, not full identity.
...
GoAnon should only mark an EUDI-compatible flow as `Privacy Grade A` if:
...
If a wallet or national implementation requires contacting an issuer/government endpoint every time a user proves age, GoAnon must not label that flow as strong privacy.
