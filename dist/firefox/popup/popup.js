...
var GOANON_PROTOCOL_VERSION, STRONG_PRIVACY_LABEL;
var init_protocol = __esm({
  "src/protocol.ts"() {
    GOANON_PROTOCOL_VERSION = "goanon.verify.v1";
    STRONG_PRIVACY_LABEL = Object.freeze({
      grade: "A",
...
async function fflonkSetup(r1csFilename, ptauFilename, zkeyFilename, logger) {
  ...
  async function writeSigma(fdZKey) {
    ...
    for (let i = 0; i < settings.nVars; i++) {
      ...
      if (logger && i !== 0 && i % 5e5 === 0)
        logger.info(`      writing sigma phase2: ${i}/${settings.nVars}`);
    }
    if (globalThis.gc)
      globalThis.gc();
    for (let i = 0; i < 3; i++) {
  ...
  function computeK1K2() {
    let k12 = Fr.two;
    while (isIncluded(k12, [], settings.cirPower))
      Fr.add(k12, Fr.one);
    let k22 = Fr.add(k12, Fr.one);
    while (isIncluded(k22, [k12], settings.cirPower))
      Fr.add(k22, Fr.one);
    return [k12, k22];
    function isIncluded(k, kArr, pow2) {
...
__export(engine_exports, {
  DEFAULT_AGE_THRESHOLD_YEARS: () => DEFAULT_AGE_THRESHOLD_YEARS,
  PROTOCOL_VERSION: () => PROTOCOL_VERSION,
  buildTestCredential: () => buildTestCredential,
  computeCommitment: () => computeCommitment,
  generateAgeProof: () => generateAgeProof,
  loadCredential: () => loadCredential,
  storeCredential: () => storeCredential,
  verifyAgeProof: () => verifyAgeProof
});
...
async function storeCredential(credential, passphrase) {
...
async function verifyAgeProof(proof, vkeyJson, options = 300) {
  const snarkjs = await getSnarkjs();
  const opts = typeof options === "number" ? { maxAgeSeconds: options } : options;
  const maxAgeSeconds = opts.maxAgeSeconds ?? 300;
  const proofAge = (Date.now() - new Date(proof.generated_at).getTime()) / 1e3;
  ...
  if (proofDate > /* @__PURE__ */ new Date()) {
  ...
  try {
    ...
    return valid ? { valid: true } : { valid: false, reason: "Cryptographic verification failed" };
  } catch (err) {
    return { valid: false, reason: `Verification error: ${err.message}` };
  }
}
async function buildTestCredential(birthdate, issuer = "local-test") {
  const date = /* @__PURE__ */ new Date(birthdate + "T00:00:00Z");
  const birthdate_days = Math.floor((date.getTime() - UNIX_EPOCH) / (1e3 * 60 * 60 * 24));
...
async function computeCommitment(birthdate_days, salt) {
...
var PROTOCOL_VERSION, DEFAULT_AGE_THRESHOLD_YEARS, DAYS_PER_YEAR, UNIX_EPOCH;
var init_engine = __esm({
  "src/engine.ts"() {
    init_protocol();
    PROTOCOL_VERSION = GOANON_PROTOCOL_VERSION;
    DEFAULT_AGE_THRESHOLD_YEARS = 18;
    DAYS_PER_YEAR = 365.25;
    UNIX_EPOCH = (/* @__PURE__ */ new Date("1970-01-01T00:00:00Z")).getTime();
  }
});
...
var ISSUERS = [
  ...
  },
  {
    ...
    badge: "Testing only",
    openAccess: true,
    description: "For local development only. Not real age verification and not accepted as a legal identity credential.",
    privacyNotes: [
...
async function connectManual() {
  return new Promise((resolve, reject) => {
    const handler = (e2) => {
      ...
      const { credential, error } = e2.detail;
      if (error)
...
async function birthdateStringToCredential(birthdateStr, issuer) {
  const { buildTestCredential: buildTestCredential2 } = await Promise.resolve().then(() => (init_engine(), engine_exports));
  return buildTestCredential2(birthdateStr, issuer);
}
...
function navigate(view) {
  document.querySelectorAll(".view").forEach((el2) => el2.classList.remove("active"));
...
async function init() {
  ...
  if (params.get("unlock") === "1") {
    ...
    if (unlockSite)
      unlockSite.textContent = `for ${site}`;
    navigate("unlock");
    return;
  }
  navigate("home");
...
function renderHome() {
  ...
  const removeBtn = document.getElementById("btn-remove-credential");
  ...
  if (state.credential) {
    ...
    flag.textContent = issuerMeta?.flag ?? "\u{1F4C4}";
    name.textContent = issuerMeta?.name ?? state.credential.issuer;
    id.textContent = `${state.credential.issuer} \xB7 privacy ${state.credential.privacy_grade ?? "A"} \xB7 local proof`;
    stored.textContent = formatDate(state.credential.stored_at);
    proofs.textContent = String(state.credential.proof_count);
    last.textContent = state.credential.last_used ? formatRelative(state.credential.last_used) : "Never";
...
function bindPassphraseEvents() {
  const input1 = document.getElementById("input-passphrase");
  const input2 = document.getElementById("input-passphrase2");
  const btn = document.getElementById("btn-connect-issuer");
...
async function startIssuerConnection() {
  ...
  try {
    const credential = issuer.id === "manual" && state.pendingManualCredential ? state.pendingManualCredential : await issuer.connect();
...
function bindManualImportEvents() {
  ...
  textarea.addEventListener("input", () => {
    try {
      ...
      if (!parsed.birthdate)
        throw new Error("Missing 'birthdate' field");
...
function renderSettings() {
  ...
  const proofTtl = document.getElementById("proof-ttl");
  const ageThreshold = document.getElementById("age-threshold");
...
function bindSettingsEvents() {
  ...
  exportBtn.addEventListener("click", () => {
    ...
    a.href = url;
    a.download = `goanon-activity-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.json`;
    a.click();
    URL.revokeObjectURL(url);
  });
  nukeBtn.addEventListener("click", async () => {
    if (!confirm("Remove your credential and clear all goanon data? This cannot be undone."))
      return;
...
function bindEvents() {
  ...
  document.getElementById("btn-remove-credential").addEventListener("click", async () => {
    if (!confirm("Remove your stored credential?"))
      return;
  ...
  document.getElementById("unlock-passphrase").addEventListener("keydown", (e2) => {
    if (e2.key === "Enter")
      handleUnlockConfirm();
  });
