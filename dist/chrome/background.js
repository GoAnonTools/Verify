...
function toRprLE(buff, o, e2, n8) {
  ...
  for (let i = v.length * 4; i < n8; i++)
    buff[i] = toNumber$1(band(shiftRight(e2, i * 8), 255));
}
      ...
      for (let j = 0; j < nGroups; j++) {
        for (let k = 0; k < nChunksPerGroup / 2; k++) {
          ...
          opPromises.push(tm.queueAction(task).then((r) => {
            if (logger)
              logger.debug(`${loggerTxt}: fft ${bits2} join  ${i}/${bits2}  ${j + 1}/${nGroups} ${k}/${nChunksPerGroup / 2}`);
            return r;
          }));
        }
      }
...
var init_browser_esm = __esm({
  "node_modules/ffjavascript/build/browser.esm.js"() {
    ...
    build_f1m = function buildF1m(module, _q, _prefix, _intPrefix) {
      ...
      let _nqr = 2n;
      if (isPrime(q)) {
        while (modPow$1(_nqr, _e, q) !== _minusOne)
          _nqr = _nqr + 1n;
      }
      let s2 = 0;
      let _t = _minusOne;
...
async function sectionIsEqual(fd1, sections1, fd2, sections2, idSection) {
  ...
  if (sections1[idSection][0].size != sections2[idSection][0].size)
    return false;
...
function hashIsEqual(h1, h2) {
  if (h1.byteLength != h2.byteLength)
    return false;
...
async function sameRatio$2(curve2, g1s, g1sx, g2s, g2sx) {
  if (curve2.G1.isZero(g1s))
    return false;
  if (curve2.G1.isZero(g1sx))
    return false;
  if (curve2.G2.isZero(g2s))
    return false;
  if (curve2.G2.isZero(g2sx))
    return false;
...
async function writeContribution$1(fd, curve2, c) {
  ...
  if (c.name) {
    ...
    for (let i = 0; i < nameData.byteLength; i++)
      params.push(nameData[i]);
  }
  if (c.type == 1) {
    params.push(2);
    ...
    for (let i = 0; i < c.beaconHash.byteLength; i++)
      params.push(c.beaconHash[i]);
  }
  if (params.length > 0) {
...
async function buildABC1(curve2, zkey2, witness, coeffs, logger) {
  ...
  for (let i = 0; i < zkey2.domainSize; i++) {
    if (logger && i % 1e6 == 0)
      logger.debug(`QAP C: ${i}/${zkey2.domainSize}`);
    outBuffC.set(
      curve2.Fr.mul(
...
async function readPTauHeader(fd, sections) {
  if (!sections[1])
    throw new Error(fd.fileName + ": File has no  header");
  ...
  if (curve2.F1.n64 * 8 != n8)
    throw new Error(fd.fileName + ": Invalid size");
...
async function readContributions(fd, curve2, sections) {
  if (!sections[7])
    throw new Error(fd.fileName + ": File has no  contributions");
  if (sections[7][0].length > 1)
    throw new Error(fd.fileName + ": File has more than one contributions section");
  ...
  if (fd.pos - sections[7][0].p != sections[7][0].size)
...
async function writeContribution(fd, curve2, contribution) {
  ...
  if (contribution.type == 1) {
    ...
    for (let i = 0; i < contribution.beaconHash.byteLength; i++)
      params.push(contribution.beaconHash[i]);
  }
  if (params.length > 0) {
    const paramsBuff = new Uint8Array(params);
    await fd.writeULE32(paramsBuff.byteLength);
...
function calculateFirstChallengeHash(curve2, power, logger) {
  if (logger)
    logger.debug("Calculating First Challenge Hash");
  ...
  n = 2 ** power * 2 - 1;
  if (logger)
    logger.debug("Calculate Initial Hash: tauG1");
  hashBlock(vG1, n);
  n = 2 ** power;
  if (logger)
    logger.debug("Calculate Initial Hash: tauG2");
  hashBlock(vG2, n);
  if (logger)
    logger.debug("Calculate Initial Hash: alphaTauG1");
  hashBlock(vG1, n);
  if (logger)
    logger.debug("Calculate Initial Hash: betaTauG1");
  hashBlock(vG1, n);
  hasher.update(vG2);
  ...
  function hashBlock(buff, n2) {
    ...
    for (let i = 0; i < nBlocks; i++) {
      ...
      if (logger)
        logger.debug("Initial hash: " + i * blockSize);
    }
    for (let i = 0; i < rem; i++) {
      hasher.update(buff);
    }
...
async function newAccumulator(curve2, power, fileName, logger) {
  ...
  if (logger)
    logger.debug(formatHash(blake2b.create({ dkLen: 64 }).digest(), "Blank Contribution Hash:"));
  if (logger)
    logger.info(formatHash(firstChallengeHash, "First Contribution Hash:"));
...
async function exportChallenge(pTauFilename, challengeFilename, logger) {
  ...
  if (logger)
    logger.info(formatHash(lastResponseHash, "Last Response Hash: "));
  if (logger)
    logger.info(formatHash(curChallengeHash, "New Challenge Hash: "));
  ...
  if (!hashIsEqual(curChallengeHash, calcCurChallengeHash)) {
    if (logger)
      logger.info(formatHash(calcCurChallengeHash, "Calc Curret Challenge Hash: "));
    if (logger)
      logger.error("PTau file is corrupted. Calculated new challenge hash does not match with the eclared one");
  ...
  async function exportSection(sectionId, groupName, nPoints, sectionName) {
    ...
    for (let i = 0; i < nPoints; i += nPointsChunk) {
      if (logger)
        logger.debug(`Exporting ${sectionName}: ${i}/${nPoints}`);
...
async function importResponse(oldPtauFilename, contributionFilename, newPTauFilename, name, importPoints, logger) {
  ...
  if (logger)
    logger.info(formatHash(hashResponse, "Contribution Response Hash imported: "));
  if (importPoints) {
    ...
    if (logger)
      logger.info(formatHash(currentContribution.nextChallenge, "Next Challenge Hash: "));
  } else {
  ...
  async function processSectionImportPoints(fdFrom, fdTo, groupName, sectionId, nPoints, singularPointIndexes, sectionName) {
    ...
    for (let i = 0; i < nPoints; i += nPointsChunk) {
      if (logger)
        logger.debug(`Importing ${sectionName}: ${i}/${nPoints}`);
      const n = Math.min(nPoints - i, nPointsChunk);
  ...
  async function hashSection(nextChallengeHasher, fdTo, groupName, sectionId, nPoints, sectionName, logger2) {
    ...
    for (let i = 0; i < nPoints; i += nPointsChunk) {
      if (logger2)
        logger2.debug(`Hashing ${sectionName}: ${i}/${nPoints}`);
      const n = Math.min(nPoints - i, nPointsChunk);
...
async function verifyContribution(curve2, cur, prev2, logger) {
  ...
  if (cur.type == 1) {
    ...
    if (!curve2.G1.eq(cur.key.tau.g1_s, beaconKey.tau.g1_s)) {
      if (logger)
        logger.error(`BEACON key (tauG1_s) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
      return false;
    }
    if (!curve2.G1.eq(cur.key.tau.g1_sx, beaconKey.tau.g1_sx)) {
      if (logger)
        logger.error(`BEACON key (tauG1_sx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
      return false;
    }
    if (!curve2.G2.eq(cur.key.tau.g2_spx, beaconKey.tau.g2_spx)) {
      if (logger)
        logger.error(`BEACON key (tauG2_spx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
      return false;
    }
    if (!curve2.G1.eq(cur.key.alpha.g1_s, beaconKey.alpha.g1_s)) {
      if (logger)
        logger.error(`BEACON key (alphaG1_s) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
      return false;
    }
    if (!curve2.G1.eq(cur.key.alpha.g1_sx, beaconKey.alpha.g1_sx)) {
      if (logger)
        logger.error(`BEACON key (alphaG1_sx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
      return false;
    }
    if (!curve2.G2.eq(cur.key.alpha.g2_spx, beaconKey.alpha.g2_spx)) {
      if (logger)
        logger.error(`BEACON key (alphaG2_spx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
      return false;
    }
    if (!curve2.G1.eq(cur.key.beta.g1_s, beaconKey.beta.g1_s)) {
      if (logger)
        logger.error(`BEACON key (betaG1_s) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
      return false;
    }
    if (!curve2.G1.eq(cur.key.beta.g1_sx, beaconKey.beta.g1_sx)) {
      if (logger)
        logger.error(`BEACON key (betaG1_sx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
      return false;
    }
    if (!curve2.G2.eq(cur.key.beta.g2_spx, beaconKey.beta.g2_spx)) {
      if (logger)
        logger.error(`BEACON key (betaG2_spx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
      return false;
    }
  }
...
async function verify(tauFilename, logger) {
  ...
  if (logger)
    logger.debug("power: 2**" + power);
  if (logger)
    logger.debug("Computing initial contribution hash");
  ...
  if (contrs.length == 0) {
    if (logger)
      logger.error("This file has no contribution! It cannot be used in production");
    return false;
  }
  let prevContr;
  if (contrs.length > 1) {
  ...
  if (logger)
    logger.debug("Validating contribution #" + contrs[contrs.length - 1].id);
  ...
  if (logger)
    logger.debug("Verifying powers in tau*G1 section");
  const rTau1 = await processSection(2, "G1", "tauG1", 2 ** power * 2 - 1, [0, 1], logger);
  sr = await sameRatio$1(curve2, rTau1.R1, rTau1.R2, curve2.G2.g, curContr.tauG2);
  if (sr !== true) {
    if (logger)
      logger.error("tauG1 section. Powers do not match");
    return false;
  }
  if (!curve2.G1.eq(curve2.G1.g, rTau1.singularPoints[0])) {
    if (logger)
      logger.error("First element of tau*G1 section must be the generator");
    return false;
  }
  if (!curve2.G1.eq(curContr.tauG1, rTau1.singularPoints[1])) {
    if (logger)
      logger.error("Second element of tau*G1 section does not match the one in the contribution section");
    return false;
  }
  if (logger)
    logger.debug("Verifying powers in tau*G2 section");
  const rTau2 = await processSection(3, "G2", "tauG2", 2 ** power, [0, 1], logger);
  sr = await sameRatio$1(curve2, curve2.G1.g, curContr.tauG1, rTau2.R1, rTau2.R2);
  if (sr !== true) {
    if (logger)
      logger.error("tauG2 section. Powers do not match");
    return false;
  }
  if (!curve2.G2.eq(curve2.G2.g, rTau2.singularPoints[0])) {
    if (logger)
      logger.error("First element of tau*G2 section must be the generator");
    return false;
  }
  if (!curve2.G2.eq(curContr.tauG2, rTau2.singularPoints[1])) {
    if (logger)
      logger.error("Second element of tau*G2 section does not match the one in the contribution section");
    return false;
  }
  if (logger)
    logger.debug("Verifying powers in alpha*tau*G1 section");
  const rAlphaTauG1 = await processSection(4, "G1", "alphatauG1", 2 ** power, [0], logger);
  sr = await sameRatio$1(curve2, rAlphaTauG1.R1, rAlphaTauG1.R2, curve2.G2.g, curContr.tauG2);
  if (sr !== true) {
    if (logger)
      logger.error("alphaTauG1 section. Powers do not match");
    return false;
  }
  if (!curve2.G1.eq(curContr.alphaG1, rAlphaTauG1.singularPoints[0])) {
    if (logger)
      logger.error("First element of alpha*tau*G1 section (alpha*G1) does not match the one in the contribution section");
    return false;
  }
  if (logger)
    logger.debug("Verifying powers in beta*tau*G1 section");
  const rBetaTauG1 = await processSection(5, "G1", "betatauG1", 2 ** power, [0], logger);
  sr = await sameRatio$1(curve2, rBetaTauG1.R1, rBetaTauG1.R2, curve2.G2.g, curContr.tauG2);
  if (sr !== true) {
    if (logger)
      logger.error("betaTauG1 section. Powers do not match");
    return false;
  }
  if (!curve2.G1.eq(curContr.betaG1, rBetaTauG1.singularPoints[0])) {
    if (logger)
      logger.error("First element of beta*tau*G1 section (beta*G1) does not match the one in the contribution section");
    return false;
  }
  const betaG2 = await processSectionBetaG2(logger);
  if (!curve2.G2.eq(curContr.betaG2, betaG2)) {
    if (logger)
      logger.error("betaG2 element in betaG2 section does not match the one in the contribution section");
    return false;
  }
  ...
  if (power == ceremonyPower) {
    if (!hashIsEqual(nextContributionHash, curContr.nextChallenge)) {
      if (logger)
        logger.error("Hash of the values does not match the next challenge of the last contributor in the contributions section");
      return false;
    }
  }
  if (logger)
    logger.info(formatHash(nextContributionHash, "Next challenge hash: "));
  ...
  async function processSection(idSection, groupName, sectionName, nPoints, singularPointIndexes, logger2) {
    ...
    for (let i = 0; i < nPoints; i += MAX_CHUNK_SIZE) {
      if (logger2)
        logger2.debug(`points relations: ${sectionName}: ${i}/${nPoints} `);
  ...
  async function verifyLagrangeEvaluations(gName, tauSection, lagrangeSection, sectionName, logger2) {
    if (logger2)
      logger2.debug(`Verifying phase2 calculated values ${sectionName}...`);
    ...
    async function verifyPower(p) {
      ...
      if (logger2)
        logger2.debug(`Creating random numbers Powers${p}...`);
      ...
      if (logger2)
        logger2.debug(`reading points Powers${p}...`);
      ...
      if (!G.eq(resTau, resLagrange)) {
        if (logger2)
          logger2.error("Phase2 caclutation does not match with powers of tau");
        return false;
      }
      return true;
    }
...
async function applyKeyToSection(fdOld, sections, fdNew, idSection, curve2, groupName, first, inc, sectionName, logger) {
  ...
  for (let i = 0; i < nPoints; i += MAX_CHUNK_SIZE) {
    if (logger)
      logger.debug(`Applying key: ${sectionName}: ${i}/${nPoints}`);
    const n = Math.min(nPoints - i, MAX_CHUNK_SIZE);
    let buff;
...
async function challengeContribute(curve2, challengeFilename, responseFileName, entropy, logger) {
  ...
  if (logger)
    logger.info(formatHash(claimedHash, "Claimed Previous Response Hash: "));
  const challengeHash = challengeHasher.digest();
  if (logger)
    logger.info(formatHash(challengeHash, "Current Challenge Hash: "));
  const key = createPTauKey(curve2, challengeHash, rng);
  if (logger) {
  ...
  const responseHash = responseHasher.digest();
  if (logger)
    logger.info(formatHash(responseHash, "Contribution Response Hash: "));
  await fdTo.close();
  await fdFrom.close();
}
async function beacon$1(oldPtauFilename, newPTauFilename, name, beaconHashStr, numIterationsExp, logger) {
  ...
  if (beaconHash.byteLength == 0 || beaconHash.byteLength * 2 != beaconHashStr.length) {
    if (logger)
      logger.error("Invalid Beacon Hash. (It must be a valid hexadecimal sequence)");
    return false;
  }
  if (beaconHash.length >= 256) {
    if (logger)
      logger.error("Maximum length of beacon hash is 255 bytes");
    return false;
  }
  numIterationsExp = parseInt(numIterationsExp);
  if (numIterationsExp < 10 || numIterationsExp > 63) {
    if (logger)
      logger.error("Invalid numIterationsExp. (Must be between 10 and 63)");
    return false;
  }
  const { fd: fdOld, sections } = await readBinFile(oldPtauFilename, "ptau", 1);
  ...
  if (power != ceremonyPower) {
    if (logger)
      logger.error("This file has been reduced. You cannot contribute into a reduced file.");
    return false;
  }
  if (sections[12]) {
    if (logger)
      logger.warn("Contributing into a file that has phase2 calculated. You will have to prepare phase2 again.");
  }
  const contributions = await readContributions(fdOld, curve2, sections);
  ...
  if (logger)
    logger.info(formatHash(curContribution.nextChallenge, "Next Challenge Hash: "));
  ...
  async function processSection(sectionId, groupName, NPoints, first, inc, sectionName, logger2) {
    ...
    for (let i = 0; i < NPoints; i += chunkSize) {
      if (logger2)
        logger2.debug(`applying key${sectionName}: ${i}/${NPoints}`);
      const n = Math.min(NPoints - i, chunkSize);
...
async function contribute(oldPtauFilename, newPTauFilename, name, entropy, logger) {
  ...
  if (sections[12]) {
    if (logger)
      logger.warn("WARNING: Contributing into a file that has phase2 calculated. You will have to prepare phase2 again.");
  }
  const contributions = await readContributions(fdOld, curve2, sections);
  const curContribution = {
    name,
  ...
  async function processSection(sectionId, groupName, NPoints, first, inc, sectionName) {
    ...
    for (let i = 0; i < NPoints; i += chunkSize) {
      if (logger)
        logger.debug(`processing: ${sectionName}: ${i}/${NPoints}`);
      const n = Math.min(NPoints - i, chunkSize);
      const buffIn = await fdOld.read(n * sG);
      const buffOutLEM = await G.batchApplyKey(buffIn, t, inc);
      const promiseWrite = fdNew.write(buffOutLEM);
  ...
  async function hashSection(fdTo, groupName, sectionId, nPoints, sectionName) {
    ...
    for (let i = 0; i < nPoints; i += nPointsChunk) {
      if (logger && i)
        logger.debug(`Hashing ${sectionName}: ` + i);
      const n = Math.min(nPoints - i, nPointsChunk);
      const buffLEM = await fdTo.read(n * sG);
      const buffU = await G.batchLEMtoU(buffLEM);
      nextChallengeHasher.update(buffU);
...
async function exportJson(pTauFilename, verbose) {
  ...
  async function exportLagrange(sectionId, groupName, sectionName) {
    ...
    for (let p = 0; p <= power; p++) {
      if (verbose)
        console.log(`${sectionName}: Power: ${p}`);
      res[p] = [];
      const nPoints = 2 ** p;
      for (let i = 0; i < nPoints; i++) {
        if (verbose && i && i % 1e4 == 0)
          console.log(`${sectionName}: ${i}/${nPoints}`);
        const buff = await fd.read(sG);
...
async function readR1csHeader(fd, sections, singleThread) {
  ...
  if (options.F) {
    if (options.F.p != res.prime)
      throw new Error("Different Prime");
    res.F = options.F;
...
async function readConstraints(fd, sections, r1cs2, logger, loggerCtx) {
  ...
  for (let i = 0; i < r1cs2.nConstraints; i++) {
    if (options.logger && i % 1e5 == 0)
      options.logger.info(`${options.loggerCtx}: Loading constraints: ${i}/${r1cs2.nConstraints}`);
...
async function readMap(fd, sections, r1cs2, logger, loggerCtx) {
  ...
  for (let i = 0; i < r1cs2.nVars; i++) {
    if (options.logger && i % 1e4 == 0)
      options.logger.info(`${options.loggerCtx}: Loading map: ${i}/${r1cs2.nVars}`);
    const idx = readULE64();
    map.push(idx);
  }
  return map;
...
async function readCustomGatesUsesSection(fd, sections, options) {
  ...
  for (let i = 0; i < nCustomGateUses; i++) {
    if (options.logger && i % 1e5 == 0)
      options.logger.info(`${options.loggerCtx}: Loading custom gate uses: ${i}/${nCustomGateUses}`);
    let c = {};
...
async function r1csInfo(r1csName, logger) {
  ...
  if (Scalar.eq(cir.prime, bn128r2)) {
    if (logger)
      logger.info("Curve: bn-128");
  } else if (Scalar.eq(cir.prime, bls12381r2)) {
    if (logger)
      logger.info("Curve: bls12-381");
  } else {
    if (logger)
      logger.info(`Unknown Curve. Prime: ${Scalar.toString(cir.prime)}`);
  }
  if (logger)
...
async function wtnsDebug(_input, wasmFileName, wtnsFileName, symName, options, logger) {
  ...
  if (options.set) {
    if (!sym)
      sym = await loadSymbols(symName);
    wcOps.logSetSignal = function(labelIdx, value) {
      if (logger)
        logger.info("SET " + sym.labelIdx2Name[labelIdx] + " <-- " + value.toString());
    };
  }
  if (options.get) {
    if (!sym)
    ...
    wcOps.logGetSignal = function(varIdx, value) {
      if (logger)
        logger.info("GET " + sym.labelIdx2Name[varIdx] + " --> " + value.toString());
    };
  }
  if (options.trigger) {
    if (!sym)
    ...
    wcOps.logStartComponent = function(cIdx) {
      if (logger)
        logger.info("START: " + sym.componentIdx2Name[cIdx]);
    };
    wcOps.logFinishComponent = function(cIdx) {
      if (logger)
        logger.info("FINISH: " + sym.componentIdx2Name[cIdx]);
    };
  }
  wcOps.sym = sym;
...
async function wtnsCheck(r1csFilename, wtnsFilename, logger) {
  if (logger)
    logger.info("WITNESS CHECKING STARTED");
  if (logger)
    logger.info("> Reading r1cs file");
  const {
    fd: fdR1cs,
  ...
  if (logger)
    logger.info("> Reading witness file");
  const {
    fd: fdWtns,
  ...
  if (logger)
    logger.info("> Checking witness correctness");
  let bR1csPos = 0;
  let res = true;
  for (let i = 0; i < r1cs2.nConstraints; i++) {
    if (logger && i !== 0 && i % 5e5 === 0) {
      logger.info(`\xB7\xB7\xB7 processing r1cs constraints ${i}/${r1cs2.nConstraints}`);
    }
    const lcA = readLC();
    const lcB = readLC();
    const lcC = readLC();
    ...
    if (!Fr.eq(Fr.sub(Fr.mul(evalA, evalB), evalC), Fr.zero)) {
      logger.warn("\xB7\xB7\xB7 aborting checking process at constraint " + i);
      res = false;
      break;
    }
  }
...
async function newZKey(r1csName, ptauName, zkeyName, logger) {
  ...
  if (r1cs2.prime != curve2.r) {
    if (logger)
      logger.error("r1cs curve does not match powers of tau ceremony curve");
    return -1;
  }
  const cirPower = log22(r1cs2.nConstraints + r1cs2.nPubInputs + r1cs2.nOutputs + 1 - 1) + 1;
  if (cirPower > power) {
    if (logger)
      logger.error(`circuit too big for this power of tau ceremony. ${r1cs2.nConstraints}*2 > 2**${power}`);
    return -1;
  }
  if (!sectionsPTau[12]) {
    if (logger)
      logger.error("Powers of tau is not prepared.");
    return -1;
  }
  ...
  await endWriteSection(fdZKey);
  if (logger)
    logger.info(formatHash(csHash, "Circuit hash: "));
  await fdZKey.close();
  await fdR1cs.close();
  await fdPTau.close();
  return csHash;
  async function writeHs() {
    ...
    if (cirPower < curve2.Fr.s) {
      ...
      for (let i = 0; i < domainSize; i++) {
        if (logger && i % 1e4 == 0)
          logger.debug(`splitting buffer: ${i}/${domainSize}`);
  ...
  async function processConstraints() {
    ...
    for (let c = 0; c < r1cs2.nConstraints; c++) {
      if (logger && c % 1e4 == 0)
        logger.debug(`processing constraints: ${c}/${r1cs2.nConstraints}`);
      ...
      for (let i = 0; i < nA; i++) {
        ...
        if (s <= nPublic) {
          if (typeof IC[s] === "undefined")
            IC[s] = [];
          IC[s].push([l2t, l2, coefp]);
        } else {
          if (typeof C[s - nPublic - 1] === "undefined")
            C[s - nPublic - 1] = [];
      ...
      for (let i = 0; i < nB; i++) {
        ...
        const l3 = sG1 * c;
        if (typeof B1[s] === "undefined")
          B1[s] = [];
        B1[s].push([l1t, l1, coefp]);
        if (typeof B2[s] === "undefined")
          B2[s] = [];
    ...
    for (let i = 0; i < coefs.length; i++) {
      if (logger && i % 1e5 == 0)
        logger.debug(`writing coeffs: ${i}/${coefs.length}`);
      writeCoef(coefs[i]);
    }
  ...
  async function composeAndWritePoints(idSection, groupName, arr, sectionName) {
    ...
    while (i < arr.length) {
      ...
      while (i < arr.length && t < curve2.tm.concurrency) {
        if (logger)
          logger.debug(`Writing points start ${sectionName}: ${i}/${arr.length}`);
        let n = 1;
        ...
        opPromises.push(composeAndWritePointsThread(groupName, subArr, logger, sectionName).then((r) => {
          if (logger)
            logger.debug(`Writing points end ${sectionName}: ${_i}/${arr.length}`);
          return r;
        }));
        i += n;
        t++;
  ...
  async function composeAndWritePointsThread(groupName, arr, logger2, sectionName) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      for (let j = 0; j < arr[i].length; j++) {
        if (logger2 && j && j % 1e4 == 0)
          logger2.debug(`Configuring big array ${sectionName}: ${j}/${arr[i].length}`);
        bBases.set(
  ...
  async function hashHPoints() {
    ...
    for (let i = 0; i < domainSize - 1; i += CHUNK_SIZE) {
      if (logger)
        logger.debug(`HashingHPoints: ${i}/${domainSize}`);
...
async function phase2importMPCParams(zkeyNameOld, mpcparamsName, zkeyNameNew, name, logger) {
  ...
  if (!hashIsEqual(newMPCParams.csHash, oldMPCParams.csHash)) {
    if (logger)
      logger.error("Hash of the original circuit does not match with the MPC one");
    return false;
  }
  if (oldMPCParams.contributions.length > newMPCParams.contributions.length) {
    if (logger)
      logger.error("The impoerted file does not include new contributions");
    return false;
  }
  for (let i = 0; i < oldMPCParams.contributions.length; i++) {
    if (!contributionIsEqual(oldMPCParams.contributions[i], newMPCParams.contributions[i])) {
      if (logger)
        logger.error(`Previous contribution ${i} does not match`);
      return false;
    }
  }
  if (name) {
  ...
  if (nIC != zkeyHeader.nPublic + 1) {
    if (logger)
      logger.error("Invalid number of points in IC");
  ...
  if (nH != zkeyHeader.domainSize - 1) {
    if (logger)
      logger.error("Invalid number of points in H");
    await fdZKeyNew.discard();
    return false;
  }
  let buffH;
  ...
  if (nL != zkeyHeader.nVars - zkeyHeader.nPublic - 1) {
    if (logger)
      logger.error("Invalid number of points in L");
    await fdZKeyNew.discard();
    return false;
  }
  let buffL;
  ...
  if (nA != zkeyHeader.nVars) {
    if (logger)
      logger.error("Invalid number of points in A");
    await fdZKeyNew.discard();
    return false;
  }
  ...
  if (nB1 != zkeyHeader.nVars) {
    if (logger)
      logger.error("Invalid number of points in B1");
    await fdZKeyNew.discard();
    return false;
  }
  fdMPCParams.pos += sG1 * zkeyHeader.nVars;
  ...
  if (nB2 != zkeyHeader.nVars) {
    if (logger)
      logger.error("Invalid number of points in B2");
    await fdZKeyNew.discard();
    return false;
  }
  ...
  function contributionIsEqual(c1, c2) {
    if (!curve2.G1.eq(c1.deltaAfter, c2.deltaAfter))
      return false;
    if (!curve2.G1.eq(c1.delta.g1_s, c2.delta.g1_s))
      return false;
    if (!curve2.G1.eq(c1.delta.g1_sx, c2.delta.g1_sx))
      return false;
    if (!curve2.G2.eq(c1.delta.g2_spx, c2.delta.g2_spx))
      return false;
    if (!hashIsEqual(c1.transcript, c2.transcript))
      return false;
    return true;
  }
}
async function phase2verifyFromInit(initFileName, pTauFileName, zkeyFileName, logger) {
  ...
  for (let i = mpcParams.contributions.length - 1; i >= 0; i--) {
    ...
    if (logger)
      logger.info("-------------------------");
    if (logger)
      logger.info(formatHash(c.contributionHash, `contribution #${i + 1} ${c.name ? c.name : ""}:`));
    if (c.type == 1) {
      if (logger)
        logger.info(`Beacon generator: ${byteArray2hex(c.beaconHash)}`);
      if (logger)
        logger.info(`Beacon iterations Exp: ${c.numIterationsExp}`);
    }
  }
  if (logger)
    logger.info("-------------------------");
  ...
  async function sectionHasSameRatio(groupName, fd1, sections1, fd2, sections2, idSection, g2sp, g2spx, sectionName) {
    ...
    for (let i = 0; i < nPoints; i += MAX_CHUNK_SIZE) {
      if (logger)
        logger.debug(`Same ratio check ${sectionName}:  ${i}/${nPoints}`);
      const n = Math.min(nPoints - i, MAX_CHUNK_SIZE);
  ...
  async function sameRatioH() {
    ...
    for (let i = 0; i < zkey2.domainSize; i += MAX_CHUNK_SIZE) {
      if (logger)
        logger.debug(`H Verification(tau):  ${i}/${zkey2.domainSize}`);
    ...
    for (let i = 0; i < zkey2.domainSize; i += MAX_CHUNK_SIZE) {
      if (logger)
        logger.debug(`H Verification(lagrange):  ${i}/${zkey2.domainSize}`);
      const n = Math.min(zkey2.domainSize - i, MAX_CHUNK_SIZE);
...
async function phase2contribute(zkeyNameOld, zkeyNameNew, name, entropy, logger) {
  ...
  if (logger)
    logger.info(formatHash(mpcParams.csHash, "Circuit Hash: "));
  if (logger)
    logger.info(formatHash(contributionHash, "Contribution Hash: "));
...
async function zkeyExportVerificationKey(zkeyName, logger) {
  ...
  await fd.close();
  if (logger)
    logger.info("EXPORT VERIFICATION KEY FINISHED");
  return res;
}
...
async function fflonkExportSolidityVerifier(vk, templates, logger) {
  if (logger)
    logger.info("FFLONK EXPORT SOLIDITY VERIFIER STARTED");
  ...
  if (logger)
    logger.info("FFLONK EXPORT SOLIDITY VERIFIER FINISHED");
...
async function plonkSetup(r1csName, ptauName, zkeyName, logger) {
  ...
  const domainSize = 2 ** cirPower;
  if (logger)
    logger.info("Plonk constraints: " + plonkConstraints.length);
  if (cirPower > power) {
    if (logger)
      logger.error(`circuit too big for this power of tau ceremony. ${plonkConstraints.length} > 2**${power}`);
    return -1;
  }
  if (!sectionsPTau[12]) {
    if (logger)
  ...
  async function processConstraints(Fr2, r1cs3, logger2) {
    function normalize2(linearComb) {
      ...
      for (let i = 0; i < ss.length; i++) {
        if (linearComb[ss[i]] == 0n)
          delete linearComb[ss[i]];
      }
    }
    ...
    for (let c = 0; c < r1cs3.constraints.length; c++) {
      if (logger2 && c % 1e4 === 0)
        logger2.debug(`processing constraints: ${c}/${r1cs3.nConstraints}`);
  ...
  async function writeSigma(sectionNum, name) {
    ...
    for (let i = 0; i < domainSize; i++) {
      ...
      if (logger && i % 1e6 == 0)
        logger.debug(`writing ${name} phase1: ${i}/${plonkConstraints.length}`);
    }
  ...
  function getK1K2() {
    let k12 = Fr.two;
    while (isIncluded(k12, [], cirPower))
      Fr.add(k12, Fr.one);
    let k22 = Fr.add(k12, Fr.one);
    while (isIncluded(k22, [k12], cirPower))
      Fr.add(k22, Fr.one);
    return [k12, k22];
...
async function plonk16Prove(zkeyFileName, witnessFileName, logger, options) {
  const { fd: fdWtns, sections: sectionsWtns } = await readBinFile(witnessFileName, "wtns", 2);
  if (logger)
    logger.debug("> Reading witness file");
  const wtns2 = await readHeader(fdWtns, sectionsWtns);
  if (logger)
    logger.debug("> Reading zkey file");
  ...
  if (logger)
    logger.debug("> Reading witness file data");
  const buffWitness = await readSection(fdWtns, sectionsWtns, 2);
  ...
  if (logger)
    logger.debug(`> Reading Section ${ZKEY_PL_ADDITIONS_SECTION}. Additions`);
  await calculateAdditions();
  if (logger)
    logger.debug(`> Reading Section ${ZKEY_PL_SIGMA_SECTION}. Sigma1, Sigma2 & Sigma 3`);
  if (logger)
    logger.debug("\xB7\xB7\xB7 Reading Sigma polynomials ");
  ...
  if (logger)
    logger.debug("\xB7\xB7\xB7 Reading Sigma evaluations");
  ...
  if (logger)
    logger.debug(`> Reading Section ${ZKEY_PL_PTAU_SECTION}. Powers of Tau`);
  ...
  await round2();
  if (logger)
    logger.debug("> ROUND 3");
  await round3();
  if (logger)
    logger.debug("> ROUND 4");
  await round4();
  if (logger)
    logger.debug("> ROUND 5");
  await round5();
  await fdZKey.close();
  await fdWtns.close();
  ...
  if (logger)
    logger.debug("PLONK PROVER FINISHED");
  return {
  ...
  async function calculateAdditions() {
    if (logger)
      logger.debug("\xB7\xB7\xB7 Computing additions");
    ...
    for (let i = 0; i < zkey2.nAdditions; i++) {
      if (logger && 0 !== i && i % 1e5 === 0)
        logger.debug(`    addition ${i}/${zkey2.nAdditions}`);
      let offset = i * sSum;
  ...
  async function round1() {
    ...
    if (logger)
      logger.debug("> Computing A, B, C wire polynomials");
    await computeWirePolynomials();
    if (logger)
      logger.debug("> Computing A, B, C MSM");
  ...
  async function computeWirePolynomials() {
    if (logger)
      logger.debug("\xB7\xB7\xB7 Reading data from zkey file");
  ...
  async function round2() {
    if (logger)
      logger.debug("> Computing challenges beta and gamma");
    transcript.reset();
    ...
    if (logger)
      logger.debug("\xB7\xB7\xB7 challenges.beta: " + Fr.toString(challenges.beta, 16));
    transcript.reset();
    transcript.addScalar(challenges.beta);
    challenges.gamma = transcript.getChallenge();
    if (logger)
      logger.debug("\xB7\xB7\xB7 challenges.gamma: " + Fr.toString(challenges.gamma, 16));
    if (logger)
      logger.debug("> Computing Z polynomial");
    await computeZ();
    if (logger)
      logger.debug("> Computing Z MSM");
    let commitZ = await polynomials.Z.multiExponentiation(PTau, "Z");
    proof.addPolynomial("Z", commitZ);
  }
  async function computeZ() {
    if (logger)
      logger.debug("\xB7\xB7\xB7 Computing Z evaluations");
    ...
    if (logger)
      logger.debug("\xB7\xB7\xB7 Computing Z ifft");
    polynomials.Z = await Polynomial.fromEvaluations(buffers.Z, curve2, logger);
    if (logger)
      logger.debug("\xB7\xB7\xB7 Computing Z fft");
  ...
  async function round3() {
    if (logger)
      logger.debug("> Computing challenge alpha");
    transcript.reset();
    transcript.addScalar(challenges.beta);
    ...
    if (logger)
      logger.debug("\xB7\xB7\xB7 challenges.alpha: " + Fr.toString(challenges.alpha, 16));
    if (logger)
      logger.debug("> Computing T polynomial");
    await computeT();
    if (logger)
      logger.debug("> Computing T MSM");
  ...
  async function computeT() {
    if (logger)
      logger.debug(`\xB7\xB7\xB7 Reading sections ${ZKEY_PL_QL_SECTION}, ${ZKEY_PL_QR_SECTION}, ${ZKEY_PL_QM_SECTION}, ${ZKEY_PL_QO_SECTION}, ${ZKEY_PL_QC_SECTION}. Q selectors`);
    ...
    if (logger)
      logger.debug("\xB7\xB7\xB7 Computing T evaluations");
    let w = Fr.one;
    ...
    if (logger)
      logger.debug("\xB7\xB7\xB7 Computing T ifft");
    polynomials.T = await Polynomial.fromEvaluations(buffers.T, curve2, logger);
    if (logger)
      logger.debug("\xB7\xB7\xB7 Computing T / ZH");
    polynomials.T.divZh(zkey2.domainSize, 4);
    if (logger)
      logger.debug("\xB7\xB7\xB7 Computing Tz ifft");
    ...
    if (logger)
      logger.debug("\xB7\xB7\xB7 Computing T1, T2, T3 polynomials");
  ...
  async function round4() {
    if (logger)
      logger.debug("> Computing challenge xi");
    transcript.reset();
    ...
    if (logger)
      logger.debug("\xB7\xB7\xB7 challenges.xi: " + Fr.toString(challenges.xi, 16));
  ...
  async function round5() {
    if (logger)
      logger.debug("> Computing challenge v");
    transcript.reset();
    ...
    if (logger)
      logger.debug("\xB7\xB7\xB7 challenges.v: " + Fr.toString(challenges.v[1], 16));
    for (let i = 2; i < 6; i++) {
      challenges.v[i] = Fr.mul(challenges.v[i - 1], challenges.v[1]);
    }
    if (logger)
      logger.debug("> Computing linearisation polynomial R(X)");
    await computeR();
    if (logger)
      logger.debug("> Computing opening proof polynomial Wxi(X) polynomial");
    computeWxi();
    if (logger)
      logger.debug("> Computing opening proof polynomial Wxiw(X) polynomial");
    computeWxiw();
    if (logger)
      logger.debug("> Computing Wxi, Wxiw MSM");
  ...
  async function computeR() {
    ...
    if (logger)
      logger.debug("PI: " + Fr2.toString(eval_pi, 16));
    ...
    r0 = Fr2.sub(r0, e4);
    if (logger)
      logger.debug("r0: " + Fr2.toString(r0, 16));
...
async function plonkVerify(_vk_verifier, _publicSignals, _proof, logger) {
  ...
  const G1 = curve2.G1;
  if (logger)
    logger.info("PLONK VERIFIER STARTED");
  ...
  if (!evaluationsAreValid$1(curve2, proof)) {
    if (logger)
      logger.error("Proof evaluations are not valid");
    return false;
  }
...
function isWellConstructed(curve2, proof) {
  const G1 = curve2.G1;
  if (!G1.isValid(proof.A))
    return false;
  if (!G1.isValid(proof.B))
    return false;
  if (!G1.isValid(proof.C))
    return false;
  if (!G1.isValid(proof.Z))
    return false;
  if (!G1.isValid(proof.T1))
    return false;
  if (!G1.isValid(proof.T2))
    return false;
  if (!G1.isValid(proof.T3))
    return false;
  if (!G1.isValid(proof.Wxi))
    return false;
  if (!G1.isValid(proof.Wxiw))
    return false;
  return true;
}
...
async function fflonkSetup(r1csFilename, ptauFilename, zkeyFilename, logger) {
  if (logger)
    logger.info("FFLONK SETUP STARTED");
  if (globalThis.gc)
    globalThis.gc();
  if (logger)
    logger.info("> Reading PTau file");
  ...
  if (logger)
    logger.info("> Getting curve from PTau settings");
  ...
  if (logger)
    logger.info("> Processing FFlonk constraints");
  await computeFFConstraints(curve2.Fr, r1cs2, logger);
  if (globalThis.gc)
    globalThis.gc();
  settings2.cirPower = Math.max(FF_T_POL_DEG_MIN, log22(plonkConstraints.length + 2 - 1) + 1);
  settings2.domainSize = 2 ** settings2.cirPower;
  if (pTauSections[2][0].size < (settings2.domainSize * 9 + 18) * sG1) {
  ...
  if (logger) {
    ...
    logger.info(`  Circuit power: ${settings2.cirPower}`);
    logger.info(`  Domain size:   ${settings2.domainSize}`);
    logger.info(`  Vars:          ${settings2.nVars}`);
    logger.info(`  Public vars:   ${settings2.nPublic}`);
    logger.info(`  Constraints:   ${plonkConstraints.length}`);
    logger.info(`  Additions:     ${plonkAdditions.length}`);
    logger.info("----------------------------");
  }
  if (logger)
    logger.info("> computing k1 and k2");
  const [k1, k2] = computeK1K2();
  if (logger)
    logger.info("> computing w3");
  const w3 = computeW3();
  if (logger)
    logger.info("> computing w4");
  const w4 = computeW4();
  if (logger)
    logger.info("> computing w8");
  const w8 = computeW8();
  if (logger)
    logger.info("> computing wr");
  const wr = getOmegaCubicRoot(settings2.cirPower, curve2.Fr);
  await writeZkeyFile();
  await fdR1cs.close();
  await fdPTau.close();
  if (logger)
  ...
  async function computeFFConstraints(Fr2, r1cs3, logger2) {
    for (let i = 0; i < settings2.nPublic; i++) {
    ...
    for (let i = 0; i < r1cs3.nConstraints; i++) {
      ...
      const [constraints, additions] = r1csProcessor.processR1csConstraint(settings2, ...readConstraint());
  ...
  async function writeZkeyFile() {
    if (logger)
      logger.info("> Writing the zkey file");
    const fdZKey = await createBinFile(zkeyFilename, "zkey", 1, ZKEY_FF_NSECTIONS, 1 << 22, 1 << 24);
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${HEADER_ZKEY_SECTION}. Zkey Header`);
    await writeZkeyHeader(fdZKey);
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_ADDITIONS_SECTION}. Additions`);
    await writeAdditions(fdZKey);
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_A_MAP_SECTION}. A Map`);
    await writeWitnessMap(fdZKey, ZKEY_FF_A_MAP_SECTION, 0, "A map");
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_B_MAP_SECTION}. B Map`);
    await writeWitnessMap(fdZKey, ZKEY_FF_B_MAP_SECTION, 1, "B map");
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_C_MAP_SECTION}. C Map`);
    await writeWitnessMap(fdZKey, ZKEY_FF_C_MAP_SECTION, 2, "C map");
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_QL_SECTION}. QL`);
    await writeQMap(fdZKey, ZKEY_FF_QL_SECTION, 3, "QL");
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_QR_SECTION}. QR`);
    await writeQMap(fdZKey, ZKEY_FF_QR_SECTION, 4, "QR");
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_QM_SECTION}. QM`);
    await writeQMap(fdZKey, ZKEY_FF_QM_SECTION, 5, "QM");
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_QO_SECTION}. QO`);
    await writeQMap(fdZKey, ZKEY_FF_QO_SECTION, 6, "QO");
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_QC_SECTION}. QC`);
    await writeQMap(fdZKey, ZKEY_FF_QC_SECTION, 7, "QC");
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Sections ${ZKEY_FF_SIGMA1_SECTION},${ZKEY_FF_SIGMA2_SECTION},${ZKEY_FF_SIGMA3_SECTION}. Sigma1, Sigma2 & Sigma 3`);
    await writeSigma(fdZKey);
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_LAGRANGE_SECTION}. Lagrange Polynomials`);
    await writeLagrangePolynomials(fdZKey);
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_PTAU_SECTION}. Powers of Tau`);
    await writePtau(fdZKey);
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_C0_SECTION}. C0`);
    await writeC0(fdZKey);
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_HEADER_SECTION}. FFlonk Header`);
    await writeFFlonkHeader(fdZKey);
    if (globalThis.gc)
      globalThis.gc();
    if (logger)
      logger.info("> Writing the zkey file finished");
    await fdZKey.close();
  }
  ...
  async function writeAdditions(fdZKey) {
    ...
    for (let i = 0; i < plonkAdditions.length; i++) {
      if (logger && i !== 0 && i % 5e5 === 0)
        logger.info(`      writing Additions: ${i}/${plonkAdditions.length}`);
      const addition = plonkAdditions[i];
  ...
  async function writeQMap(fdZKey, sectionNum, posConstraint, name) {
    let Q = new BigBuffer(settings2.domainSize * sFr);
    for (let i = 0; i < plonkConstraints.length; i++) {
  ...
  async function writeSigma(fdZKey) {
    const sigma = new BigBuffer(sFr * settings2.domainSize * 3);
    const lastSeen = new BigArray$1(settings2.nVars);
    const firstPos = new BigArray$1(settings2.nVars);
    let w = Fr.one;
    for (let i = 0; i < settings2.domainSize; i++) {
      if (i < plonkConstraints.length) {
        buildSigma(plonkConstraints[i][0], i);
        buildSigma(plonkConstraints[i][1], settings2.domainSize + i);
        buildSigma(plonkConstraints[i][2], settings2.domainSize * 2 + i);
      } else if (i < settings2.domainSize - 2) {
        buildSigma(0, i);
        buildSigma(0, settings2.domainSize + i);
        buildSigma(0, settings2.domainSize * 2 + i);
      } else {
        sigma.set(w, i * sFr);
        sigma.set(Fr.mul(w, k1), (settings2.domainSize + i) * sFr);
        sigma.set(Fr.mul(w, k2), (settings2.domainSize * 2 + i) * sFr);
      }
      w = Fr.mul(w, Fr.w[settings2.cirPower]);
      if (logger && i !== 0 && i % 5e5 === 0) {
        logger.info(`      writing sigma phase1: ${i}/${plonkConstraints.length}`);
      }
    }
    for (let i = 0; i < settings2.nVars; i++) {
      ...
      if (logger && i !== 0 && i % 5e5 === 0)
        logger.info(`      writing sigma phase2: ${i}/${settings2.nVars}`);
    }
    if (globalThis.gc)
      globalThis.gc();
    for (let i = 0; i < 3; i++) {
      ...
      let name = "S" + (i + 1);
      polynomials[name] = await Polynomial.fromEvaluations(sigma.slice(settings2.domainSize * sFr * i, settings2.domainSize * sFr * (i + 1)), curve2, logger);
      evaluations[name] = await Evaluations.fromPolynomial(polynomials[name], 4, curve2, logger);
    ...
    function buildSigma(signalId, idx) {
      ...
      let v;
      if (idx < settings2.domainSize) {
        v = w;
      } else if (idx < 2 * settings2.domainSize) {
        v = Fr.mul(w, k1);
      } else {
        v = Fr.mul(w, k2);
      }
  ...
  async function writeLagrangePolynomials(fdZKey) {
    ...
    const l = Math.max(settings2.nPublic, 1);
    for (let i = 0; i < l; i++) {
      let buff = new BigBuffer(settings2.domainSize * sFr);
  ...
  async function writePtau(fdZKey) {
    ...
    PTau = new BigBuffer((settings2.domainSize * 9 + 18) * sG1);
    await fdPTau.readToBuffer(PTau, 0, (settings2.domainSize * 9 + 18) * sG1, pTauSections[2][0].p);
  ...
  async function writeC0(fdZKey) {
    ...
    if (polynomials.C0.degree() >= 8 * settings2.domainSize) {
  ...
  async function writeFFlonkHeader(fdZKey) {
    ...
    await fdZKey.writeULE32(n8r);
    await writeBigInt(fdZKey, primeR, n8r);
    await fdZKey.writeULE32(settings2.nVars);
    await fdZKey.writeULE32(settings2.nPublic);
    await fdZKey.writeULE32(settings2.domainSize);
    await fdZKey.writeULE32(plonkAdditions.length);
    await fdZKey.writeULE32(plonkConstraints.length);
    await fdZKey.write(k1);
    await fdZKey.write(k2);
  ...
  async function writeP4(fdZKey, buff) {
    const [coefficients, evaluations4] = await Polynomial.to4T(buff, settings2.domainSize, [], Fr);
  ...
  function computeK1K2() {
    let k12 = Fr.two;
    while (isIncluded(k12, [], settings2.cirPower))
      Fr.add(k12, Fr.one);
    let k22 = Fr.add(k12, Fr.one);
    while (isIncluded(k22, [k12], settings2.cirPower))
      Fr.add(k22, Fr.one);
    return [k12, k22];
    function isIncluded(k, kArr, pow2) {
...
async function fflonkProve(zkeyFileName, witnessFileName, logger, options) {
  if (logger)
    logger.info("FFLONK PROVER STARTED");
  if (logger)
    logger.info("> Reading witness file");
  const {
    fd: fdWtns,
  ...
  if (logger)
    logger.info("> Reading witness file data");
  const buffWitness = await readSection(fdWtns, wtnsSections, 2);
  await fdWtns.close();
  buffWitness.set(Fr.zero, 0);
  ...
  let proof = new Proof(curve2, logger);
  if (logger)
    logger.info(`> Reading Section ${ZKEY_FF_ADDITIONS_SECTION}. Additions`);
  await calculateAdditions();
  if (logger)
    logger.info(`> Reading Sections ${ZKEY_FF_SIGMA1_SECTION},${ZKEY_FF_SIGMA2_SECTION},${ZKEY_FF_SIGMA3_SECTION}. Sigma1, Sigma2 & Sigma 3`);
  if (logger)
    logger.info("\xB7\xB7\xB7 Reading Sigma polynomials ");
  polynomials.Sigma1 = new Polynomial(new BigBuffer(sDomain), curve2, logger);
  polynomials.Sigma2 = new Polynomial(new BigBuffer(sDomain), curve2, logger);
  polynomials.Sigma3 = new Polynomial(new BigBuffer(sDomain), curve2, logger);
  ...
  if (logger)
    logger.info("\xB7\xB7\xB7 Reading Sigma evaluations");
  evaluations.Sigma1 = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
  evaluations.Sigma2 = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
  evaluations.Sigma3 = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
  ...
  if (logger)
    logger.info(`> Reading Section ${ZKEY_FF_PTAU_SECTION}. Powers of Tau`);
  ...
  if (logger)
    logger.info("FFLONK PROVER FINISHED");
  return {
  ...
  async function round1() {
    ...
    if (logger)
      logger.info("> Computing A, B, C wire polynomials");
    await computeWirePolynomials();
    if (logger)
      logger.info("> Computing T0 polynomial");
    await computeT0();
    if (logger)
      logger.info("> Computing C1 polynomial");
    await computeC1();
    if (logger)
      logger.info("> Computing C1 multi exponentiation");
    let commitC1 = await polynomials.C1.multiExponentiation(PTau, "C1");
    proof.addPolynomial("C1", commitC1);
    return 0;
    async function computeWirePolynomials() {
      if (logger)
        logger.info("\xB7\xB7\xB7 Reading data from zkey file");
      buffers.A = new BigBuffer(sDomain);
      buffers.B = new BigBuffer(sDomain);
      buffers.C = new BigBuffer(sDomain);
      ...
      buffers.C = await Fr.batchToMontgomery(buffers.C);
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing A ifft");
      polynomials.A = await Polynomial.fromEvaluations(buffers.A, curve2, logger);
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing B ifft");
      polynomials.B = await Polynomial.fromEvaluations(buffers.B, curve2, logger);
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing C ifft");
      polynomials.C = await Polynomial.fromEvaluations(buffers.C, curve2, logger);
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing A fft");
      evaluations.A = await Evaluations.fromPolynomial(polynomials.A, 4, curve2, logger);
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing B fft");
      evaluations.B = await Evaluations.fromPolynomial(polynomials.B, 4, curve2, logger);
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing C fft");
      evaluations.C = await Evaluations.fromPolynomial(polynomials.C, 4, curve2, logger);
    ...
    async function computeT0() {
      if (logger)
        logger.info(`\xB7\xB7\xB7 Reading sections ${ZKEY_FF_QL_SECTION}, ${ZKEY_FF_QR_SECTION}, ${ZKEY_FF_QM_SECTION}, ${ZKEY_FF_QO_SECTION}, ${ZKEY_FF_QC_SECTION}. Q selectors`);
      evaluations.QL = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
      evaluations.QR = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
      evaluations.QM = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
      evaluations.QO = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
      ...
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing T0 evaluations");
      for (let i = 0; i < zkey2.domainSize * 4; i++) {
        if (logger && 0 !== i && i % 1e5 === 0)
          logger.info(`      T0 evaluation ${i}/${zkey2.domainSize * 4}`);
        const a = evaluations.A.getEvaluation(i);
        const b = evaluations.B.getEvaluation(i);
        const c = evaluations.C.getEvaluation(i);
        const ql = evaluations.QL.getEvaluation(i);
      ...
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing T0 ifft");
      ...
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing T0 / ZH");
  ...
  async function round2() {
    if (logger)
      logger.info("> Computing challenges beta and gamma");
    const transcript = new Keccak256Transcript(curve2);
    ...
    challenges.beta = transcript.getChallenge();
    if (logger)
      logger.info("\xB7\xB7\xB7 challenges.beta: " + Fr.toString(challenges.beta));
    transcript.reset();
    transcript.addScalar(challenges.beta);
    challenges.gamma = transcript.getChallenge();
    if (logger)
      logger.info("\xB7\xB7\xB7 challenges.gamma: " + Fr.toString(challenges.gamma));
    if (logger)
      logger.info("> Computing Z polynomial");
    await computeZ();
    if (logger)
      logger.info("> Computing T1 polynomial");
    await computeT1();
    if (logger)
      logger.info("> Computing T2 polynomial");
    await computeT2();
    if (logger)
      logger.info("> Computing C2 polynomial");
    await computeC2();
    if (logger)
      logger.info("> Computing C2 multi exponentiation");
    let commitC2 = await polynomials.C2.multiExponentiation(PTau, "C2");
    proof.addPolynomial("C2", commitC2);
    return 0;
    async function computeZ() {
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing Z evaluations");
      let numArr = new BigBuffer(sDomain);
      let denArr = new BigBuffer(sDomain);
      numArr.set(Fr.one, 0);
      denArr.set(Fr.one, 0);
      ...
      for (let i = 0; i < zkey2.domainSize; i++) {
        if (logger && 0 !== i && i % 1e5 === 0)
          logger.info(`    Z evaluation ${i}/${zkey2.domainSize}`);
        const i_sFr = i * sFr;
        const betaw = Fr.mul(challenges.beta, w);
      ...
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing Z ifft");
      polynomials.Z = await Polynomial.fromEvaluations(buffers.Z, curve2, logger);
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing Z fft");
      evaluations.Z = await Evaluations.fromPolynomial(polynomials.Z, 4, curve2, logger);
      polynomials.Z.blindCoefficients([challenges.b[9], challenges.b[8], challenges.b[7]]);
      if (polynomials.Z.degree() >= zkey2.domainSize + 3) {
        throw new Error("Z Polynomial is not well calculated");
    ...
    async function computeT1() {
      ...
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing T1 ifft");
      ...
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing T1z ifft");
    ...
    async function computeT2() {
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing T2 evaluations");
      buffers.T2 = new BigBuffer(sDomain * 4);
      buffers.T2z = new BigBuffer(sDomain * 4);
      let omega = Fr.one;
      for (let i = 0; i < zkey2.domainSize * 4; i++) {
        if (logger && 0 !== i && i % 1e5 === 0)
          logger.info(`    T2 evaluation ${i}/${zkey2.domainSize * 4}`);
        const omega2 = Fr.square(omega);
  ...
  async function round3() {
    if (logger)
      logger.info("> Computing challenge xi");
    const transcript = new Keccak256Transcript(curve2);
    transcript.addScalar(challenges.gamma);
    ...
    if (logger)
      logger.info("\xB7\xB7\xB7 challenges.xi: " + Fr.toString(challenges.xi));
    polynomials.QL = new Polynomial(new BigBuffer(sDomain), curve2, logger);
    polynomials.QR = new Polynomial(new BigBuffer(sDomain), curve2, logger);
    polynomials.QM = new Polynomial(new BigBuffer(sDomain), curve2, logger);
    polynomials.QO = new Polynomial(new BigBuffer(sDomain), curve2, logger);
    ...
    if (logger)
      logger.info("\xB7\xB7\xB7 Computing evaluations");
  ...
  async function round4() {
    ...
    challenges.alpha = transcript.getChallenge();
    if (logger)
      logger.info("\xB7\xB7\xB7 challenges.alpha: " + Fr.toString(challenges.alpha));
    if (logger)
      logger.info("> Reading C0 polynomial");
    ...
    if (logger)
      logger.info("> Computing R0 polynomial");
    computeR02();
    if (logger)
      logger.info("> Computing R1 polynomial");
    computeR12();
    if (logger)
      logger.info("> Computing R2 polynomial");
    computeR22();
    if (logger)
      logger.info("> Computing F polynomial");
    await computeF2();
    if (logger)
      logger.info("> Computing W1 multi exponentiation");
    ...
    async function computeF2() {
      if (logger)
        logger.info("\xB7\xB7\xB7 Computing F polynomial");
  ...
  async function round5() {
    if (logger)
      logger.info("> Computing challenge y");
    const transcript = new Keccak256Transcript(curve2);
    transcript.addScalar(challenges.alpha);
    transcript.addPolCommitment(proof.getPolynomial("W1"));
    challenges.y = transcript.getChallenge();
    if (logger)
      logger.info("\xB7\xB7\xB7 challenges.y: " + Fr.toString(challenges.y));
    if (logger)
      logger.info("> Computing L polynomial");
    await computeL();
    if (logger)
      logger.info("> Computing ZTS2 polynomial");
    await computeZTS2();
    ...
    if (logger)
      logger.info("> Computing W' = L / ZTS2 polynomial");
    ...
    async function computeL() {
      ...
      if (logger)
        logger.info("> Computing ZT polynomial");
      await computeZT();
...
async function fflonkVerify(_vk_verifier, _publicSignals, _proof, logger) {
  if (logger)
    logger.info("FFLONK VERIFIER STARTED");
  ...
  if (logger)
    logger.info("> Checking commitments belong to G1");
  ...
  if (logger)
    logger.info("> Checking evaluations belong to F");
  ...
  if (logger)
    logger.info("> Checking public inputs belong to F");
  ...
  if (logger)
    logger.info("> Computing J");
  const J = computeJ(curve2, proof, challenges);
  if (logger)
    logger.info("> Validate all evaluations with a pairing");
  ...
  if (logger)
    logger.info("FFLONK VERIFIER FINISHED");
  return res;
}
...
function computeR0(proof, challenges, roots, curve2, logger) {
  ...
  if (logger)
    logger.info("\xB7\xB7\xB7 Computing r0(y)");
  let res = Fr.zero;
  for (let i = 0; i < 8; i++) {
    let coefValues = [];
...
function computeR1(proof, challenges, roots, pi, curve2, logger) {
  ...
  if (logger)
    logger.info("\xB7\xB7\xB7 Computing T0(xi)");
  ...
  if (logger)
    logger.info("\xB7\xB7\xB7 Computing C1(h_1\u03C9_4^i) values");
  let res = Fr.zero;
  for (let i = 0; i < 4; i++) {
...
function computeR2(proof, challenges, roots, lagrange1, vk, curve2, logger) {
  ...
  if (logger)
    logger.info("\xB7\xB7\xB7 Computing C2(h_2\u03C9_3^i) values");
  let res = Fr.zero;
  for (let i = 0; i < 3; i++) {
  ...
  if (logger)
    logger.info("\xB7\xB7\xB7 Computing C2(h_3\u03C9_3^i) values");
  for (let i = 0; i < 3; i++) {
  ...
  "node_modules/snarkjs/build/browser.esm.js"() {
    ...
    FastFile = class {
      ...
      async write(buff, pos) {
        ...
        if (self.totalSize < pos + buff.byteLength)
          self.totalSize = pos + buff.byteLength;
        ...
        for (let i = firstPage; i <= lastPage; i++)
          pagePromises.push(self._loadPage(i));
        self._triggerLoad();
        let p = firstPage;
      ...
      _tryClose() {
        ...
        if (p >= 0 || self.writing || self.reading || self.pendingLoads.length > 0)
          return;
        self.pendingClose();
      }
      close() {
    ...
    blake2b = /* @__PURE__ */ wrapConstructorWithOpts((opts) => new BLAKE2b(opts));
...
`);
    ...
    groth16 = /* @__PURE__ */ Object.freeze({
      __proto__: null,
    ...
    powersoftau = /* @__PURE__ */ Object.freeze({
      __proto__: null,
      newAccumulator,
      exportChallenge,
      importResponse,
    ...
    Polynomial = class _Polynomial {
      ...
      getCoef(index) {
        ...
        if (i_n8 + this.Fr.n8 > this.coef.byteLength)
          return this.Fr.zero;
      ...
      divByVanishing2(m, beta) {
        ...
        for (let k = 0; k < nThreads; k++) {
          ...
          for (let i = k === 0 ? nElementsLast : nElementsChunk; i > 0; i--) {
            let idxDst = i - 1;
            if (k !== 0)
              idxDst += (k - 1) * nElementsChunk + nElementsLast;
            let idxSrc = idxDst + m;
    ...
    r1csConstraintProcessor = class {
      ...
      processR1csConstraint(settings2, lcA, lcB, lcC) {
      ...
      normalizeLinearCombination(linCom) {
        ...
        for (let i = 0; i < signalIds.length; i++) {
          if (this.Fr.isZero(linCom[signalIds[i]]))
      ...
      reduceCoefs(settings2, constraintsArr, additionsArr, linCom, maxC) {
        const res = {
          k: this.Fr.zero,
          signals: [],
          coefs: []
      ...
      processR1csAdditionConstraint(settings2, linCom) {
        const constraintsArr = [];
        const additionsArr = [];
        const C = this.reduceCoefs(settings2, constraintsArr, additionsArr, linCom, 3);
        const constraints = this.fnGetAdditionConstraint(
          C.signals[0],
          C.signals[1],
          C.signals[2],
      ...
      processR1csMultiplicationConstraint(settings2, lcA, lcB, lcC) {
        const constraintsArr = [];
        const additionsArr = [];
        const A = this.reduceCoefs(settings2, constraintsArr, additionsArr, lcA, 1);
        const B = this.reduceCoefs(settings2, constraintsArr, additionsArr, lcB, 1);
        const C = this.reduceCoefs(settings2, constraintsArr, additionsArr, lcC, 1);
...
// src/engine.ts
var PROTOCOL_VERSION = GOANON_PROTOCOL_VERSION;
var DEFAULT_AGE_THRESHOLD_YEARS = 18;
...
async function loadCredential(stored, passphrase) {
...
async function generateAgeProof(credential, wasmPath, zkeyPath, thresholdYearsOrContext = DEFAULT_AGE_THRESHOLD_YEARS) {
  const today = /* @__PURE__ */ new Date();
  const today_days = Math.floor((today.getTime() - UNIX_EPOCH) / (1e3 * 60 * 60 * 24));
  const context = typeof thresholdYearsOrContext === "number" ? null : thresholdYearsOrContext;
  const thresholdYears = typeof thresholdYearsOrContext === "number" ? thresholdYearsOrContext : thresholdYearsOrContext.minAge ?? DEFAULT_AGE_THRESHOLD_YEARS;
  ...
  if (typeof URL === "undefined" || typeof URL.createObjectURL !== "function") {
    throw new Error(
      "Real ZK proving is not available inside this Chrome MV3 service worker yet. Use a demo credential for the alpha flow, or move proving to an offscreen document."
    );
  }
  const input = {
...
function normalizeSnarkjs(mod2) {
  if (mod2?.groth16?.fullProve)
    return mod2;
  if (mod2?.default?.groth16?.fullProve)
    return mod2.default;
  if (mod2?.snarkjs?.groth16?.fullProve)
    return mod2.snarkjs;
  return null;
}
async function getSnarkjs() {
  const globalCandidate = normalizeSnarkjs(globalThis.snarkjs);
  if (globalCandidate)
    return globalCandidate;
  const imported = await Promise.resolve().then(() => (init_browser_esm2(), browser_esm_exports));
  const candidate = normalizeSnarkjs(imported);
  if (candidate)
    return candidate;
  throw new Error("snarkjs Groth16 prover is unavailable in this build.");
}
function isDemoCredential(credential) {
  ...
  if (explicitDemoIssuers.includes(issuer))
    return true;
  if (keyId.includes(":demo"))
    return true;
...
// src/background.ts
var KEY_CREDENTIAL = "goanon_credential_v1";
var KEY_SETTINGS = "goanon_settings_v1";
var KEY_CRED_META = "goanon_credential_meta";
var passphraseResolver = null;
var pendingProofSite = null;
var settings = {
...
chrome.storage.local.get(KEY_SETTINGS).then((r) => {
  if (r[KEY_SETTINGS])
    settings = { ...settings, ...r[KEY_SETTINGS] };
});
...
async function openUnlockPopup(site) {
